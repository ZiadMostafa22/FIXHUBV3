...
## Table of Contents
1. [Use Case Diagram (4.3)](#43-use-case-diagram)
2. [Class Diagram (4.4)](#44-class-diagram)
3. [Use Case Tables (4.5.1)](#451-use-case-tables)
4. [Activity Diagrams (4.6)](#46-activity-diagrams)
5. [Sequence Diagrams (4.7)](#47-sequence-diagrams)
6. [State Diagrams (4.8)](#48-state-diagrams)
...
```mermaid
graph TB
    subgraph S[Fix-Hub System]
        MC[Manage Cars] -.->|include| AC[Add Car]
        MC -.->|include| EC[Edit Car]
        MC -.->|include| DC[Delete Car]
        
        BS[Book Service] -.->|include| SC[Select Car]
        BS -.->|include| SS[Select Services]
        BS -.->|include| SD[Schedule]
        AO[Apply Offer] -.->|extend| BS
        ...
        VJ[View Jobs]
        SJ[Start Job]
        AI2[Add Items] -.->|include| CI[Check Inventory]
        CJ[Complete Job]
        AN[Add Notes] -.->|extend| CJ
        VP[View Performance]
        
        GC[Generate Codes]
        MU[Manage Users] -.->|include| AU[Activate User]
        AR[Approve Refund]
        CO[Create Offers]
        VR[View Reports]
        MI[Manage Inventory] -.->|include| AS[Add Stock]
        MI -.->|include| VA[View Alerts]
        
        PP[Process Payment] -.->|include| SM[Select Method]
        PP -.->|include| GI[Generate Invoice]
        IR[Initiate Refund]
        PR[Profit Reports]
    end
    
    C((Customer)) --> MC & BS & TS & RS & DI & AI
    T((Technician)) --> VJ & SJ & AI2 & CJ & VP
    A((Admin)) --> GC & MU & AR & CO & VR & MI
    CS((Cashier)) --> PP & IR & PR
```
...
This simplified class diagram shows the core entities in the Fix-Hub system with essential attributes and key relationships.
...
classDiagram
    ...
    %% Core Relationships
    User "1" --o "*" Car : owns
    User "1" --o "*" Booking : creates
    User "1" --o "*" Offer : creates (Admin)
    
    Car "1" --o "*" Booking : has
    
    Booking "*" --o "1" User : belongs to
    Booking "*" --o "1" Car : for
    Booking "*" --o "*" Service : uses
    Booking "1" --o "0..1" Refund : may have
    Booking "*" --o "0..1" Offer : applies
    
    Refund "*" --o "1" Booking : for
    Refund "*" --o "1" User : requested by
    Refund "*" --o "0..1" User : approved by
...
graph TD
    ...
    ShowTypes --> SelectType["Select 'Select Type: Regular/Repair/Inspection/Emergency"]
...
sequenceDiagram
    ...
    Note over C, FS: Dashboard & Discovery
    C->>App: Open Dashboard
    par Parallel Data Fetch
        App->>FS: Query active offers
        App->>FS: Query top categories
        App->>FS: Query maintenance reminders (for user's cars)
    end
    FS-->>App: Return Offers, Categories & Reminders
    App-->>C: Display Dashboard UI
    end
    
    rect rgb(245, 245, 245)
    Note over C, FS: Service Catalog Browsing
    C->>App: Browse Categories (Oil, Engine, etc.)
    App->>FS: Query services where category=selected
    FS-->>App: Return Categorized Services
    C->>App: Search "Battery" + Filter [Brand, Price]
    App->>FS: Query services with filters
    FS-->>App: Return Filtered List
    App-->>C: Display Service Results
    end
    
    rect rgb(240, 240, 240)
    Note over C, FS: My Garage & Booking History
    C->>App: View "My Garage"
    App->>FS: Query users/uid/cars
    FS-->>App: Return Car List
    C->>App: Select Car -> View History
    App->>FS: Query bookings where carId=selected
    FS-->>App: Return Maintenance History
    C->>App: View Booking Detail -> Download Invoice
    App->>FS: getDoc(invoices/bookingId)
    FS-->>App: Return Invoice Data
    App-->>C: Show Status & Open PDF
    end
    
    rect rgb(245, 245, 245)
    Note over C, FS: Notifications
    C->>App: Open Notifications Hub
    App->>FS: Query user_notifications (unread first)
    FS-->>App: Return Status Updates & Alerts
    App-->>C: Display Notifications List
    end
```
...
sequenceDiagram
    ...
    rect rgb(240, 240, 240)
    Note over T, FS: Job Management
    T->>App: Open Dashboard
    par Dashboard Data
        App->>FS: Query available jobs (status=pending)
        App->>FS: Query "My Tasks" (assigned to me today)
    end
    FS-->>App: Return Job Streams
    App-->>T: Display My Schedule
    end
    
    rect rgb(245, 245, 245)
    Note over T, FS: Job Technical Details
    T->>App: Select Job Detail
    App->>FS: getDoc(bookings/jobId)
    App->>FS: Query car history (last 5 services)
    App->>FS: Get technical notes 
    FS-->>App: Return Comprehensive Job Data
    App-->>T: Display Technical Workspace
    end
    
    rect rgb(240, 240, 240)
    Note over T, FS: Performance Browsing
    T->>App: View My Performance
    App->>FS: Aggregate my stats (completed jobs, ratings)
    FS-->>App: Return Performance Metrics
    App-->>T: Display Monthly Earnings & Rating Stats
    end
```
...
sequenceDiagram
    ...
    rect rgb(240, 240, 240)
    Note over A, FS: System Overview 
    A->>App: Open Admin Panel
    par Real-time Stats
        App->>FS: Aggregate active bookings count
        App->>FS: Query revenue totals (today)
        App->>FS: Query count of idle technicians
    end
    FS-->>App: Return Stats Summary
    App-->>A: Display Dashboard Widgets
    end
    
    rect rgb(245, 245, 245)
    Note over A, FS: User & Staff Management
    A->>App: View User List
    App->>FS: Query all users 
    FS-->>App: Return User Profiles
    A->>App: Filter or Search by Name
    App->>FS: Query filtered users
    FS-->>App: Return Search Results
    App-->>A: Display User Directory
    end
    
    rect rgb(240, 240, 240)
    Note over A, FS: Inventory & Financial Control
    A->>App: Browse Inventory
    App->>FS: Query inventory (all items)
    App->>FS: Query low_stock_alerts
    FS-->>App: Return Stock Levels & Alerts
    A->>App: View Refund Requests
    App->>FS: Query refunds where status=requested
    FS-->>App: Return Pending Refunds
    App-->>A: Display Inventory Table & Refund List
    end
    
    rect rgb(245, 245, 245)
    Note over A, FS: Marketing & Offers
    A->>App: Manage Offers
    App->>FS: Query all offers (active & expired)
    FS-->>App: Return Offer Registry
    App-->>A: Display Marketing Hub
    end
```
...
sequenceDiagram
    ...
    rect rgb(240, 240, 240)
    Note over CS, FS: Payment Operations
    CS->>App: View Payment Queue
    App->>FS: Query bookings where status=completedPendingPayment
    FS-->>App: Return Awaiting Payments
    App-->>CS: Display Payment Queue
    CS->>App: View Refund Status
    App->>FS: Query refunds where status=approved
    FS-->>App: Return Approved Refunds
    App-->>CS: Display Refund Status
    App-->>CS: Display Operational Lists
    end
    
    rect rgb(245, 245, 245)
    Note over CS, FS: Transaction History & Archive
    CS->>App: Search Transaction (ID or Customer)
    App->>FS: Query bookings where isPaid=true + filters
    FS-->>App: Return Transaction Results
    App-->>CS: Display Transaction Results
    CS->>App: View Invoice Archive
    App->>FS: getDoc(invoices/bookingId)
    FS-->>App: Return Historical Invoice
    App-->>CS: Display Invoice Archive
    end
    
    rect rgb(240, 240, 240)
    Note over CS, FS: Daily Settlement
    CS->>App: Open Settlement Report
    App->>FS: Query paid bookings for today
    FS-->>App: Return Today's Transactions
    App->>App: Calculate Cash vs Digital totals
    App-->>CS: Display Daily Revenue & Shift Summary
    end
...
sequenceDiagram
    ...
    App->>App: Validate Code Status
    alt Code Valid
        App->>Auth: createUserWithEmailAndPassword("email, password")
        Auth-->>App: Return UID
        App->>FS: Create user with role=technician
        App->>FS: Update Code Status
        FS-->>App: Success
...
stateDiagram-v2
    ...
    Active --> END : Account Deactivated
```
...
```mermaid
stateDiagram-v2
    START --> Pending : Customer Creates Booking
    ...
    Completed --> END : Workflow Ends
    Cancelled --> END : Workflow Ends
