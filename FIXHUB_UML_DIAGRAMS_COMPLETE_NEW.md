...
### 4.7.1 User Registration (Customer & Staff)
...
sequenceDiagram
    ...
    App->>FS: Validate Invite Code (if Staff)
    App->>Auth: Create Account
    Auth-->>App: Return UID
    App->>FS: Create User Profile (Role-based)
    App-->>U: Redirect to Dashboard
```

### 4.7.2 Core Feature: Booking Lifecycle
...
sequenceDiagram
    ...
    Note over C,FS: Phase 1: Creation
    C->>FS: Create Booking (Status: Pending)
    FS-->>T: Notify: New Job Available
    
    Note over T,FS: Phase 2: Execution
    T->>FS: Start Job (Status: In-Progress)
    T->>FS: Update Services & Parts
    T->>FS: Complete Job (Status: Pending Payment)
    FS-->>CS: Notify: Payment Required
    
    Note over CS,FS: Phase 3: Payment
    CS->>FS: Process Payment (Status: Completed)
    FS-->>C: Invoice & Rating Request
```

### 4.7.3 System Browsing & Monitoring
...
sequenceDiagram
    ...
    App->>FS: Fetch Role-Specific Data (Jobs/Stats/Offers)
    FS-->>App: Return Data Stream
    App-->>User: Display Relevant View
    
    Note over App,FS: Admin Only
    FS->>App: Alert: Low Stock / Refund Request
    App-->>User: Show Emergency Notification
```
```
